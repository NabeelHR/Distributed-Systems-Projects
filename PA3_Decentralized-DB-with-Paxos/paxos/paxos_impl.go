...
func NewPaxosNode(myHostPort string, hostMap map[int]string, numNodes, srvId, numRetries int, replace bool) (PaxosNode, error) {
	...
	newNode.getCommitt = make(chan kvPair)
	...
	t := time.Now()
	elapsed := t.Sub(start)
	LOGF.Printf("====NewPaxosNode %s took %s", srvId, elapsed)
...
func (pn *paxosNode) requestsHandler() {
	for {
		...
		///////////////process get request
		//This is essentially the same as getReq. The only difference is that it serves a function
		//in order to ensure that the responses don't go back to the wrong go routine
		case p := <-pn.getCommitt:

			pn.getCommitt <- kvPair{p.k, pn.kvStore[p.k]}
		}
	}
}
...
func (pn *paxosNode) GetValue(args *paxosrpc.GetValueArgs, reply *paxosrpc.GetValueReply) error {
	rand.Seed(time.Now().UnixNano())
	t := rand.Intn(1500) //random delay incase multiple getValue RPC calls invoked simultaneuosly

	time.Sleep(time.Millisecond * time.Duration(t))
	pn.getCommitt <- kvPair{args.Key, 0}
	reply.V = (<-pn.getCommitt).v
